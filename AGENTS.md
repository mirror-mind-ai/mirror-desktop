...
- Treat [docs/project/history/nautilus-harness-transfer.md](docs/project/history/nautilus-harness-transfer.md) as the lineage record for the transition from Nautilus Harness to Mirror Desktop.
...
- `$HOME/mirror` is a production runtime dependency, not a Mirror Desktop development workspace. Mirror Desktop work may inspect and execute that released checkout but must never edit, patch, test source changes in, or commit to it.
- Any required Mirror source change must be developed in the checkout associated with Journey `mirror-dev` at `$HOME/.mirror-journeys/mirror-mind/mirror-dev`, pass Mirror's own gates and reach `$HOME/mirror` only through the official Mirror release and runtime-update path. Mirror Desktop authorization never implies production Mirror repair authority.

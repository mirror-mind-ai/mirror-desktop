[< Parent](../index.md)

# DS-005.US-1 — Restore Journey Conversation

**Status:** ✅ Done
**Type:** User Story

---

## User Story

As a user,
I want to Restore Journey Conversation,
So that I can receive the value of this story.

## Outcome

Navigator can validate Restore Journey Conversation as an observable behavior.

## Acceptance Behavior

```text
Given the user is ready for Restore Journey Conversation
When the user performs the planned action
Then the expected observable behavior is visible
And unrelated Delivery Story scope remains untouched
```

## Scope

- Restore Journey Conversation

## Out Of Scope

- Sibling Delivery Story scope.

## Validation

Navigator-visible validation route plus automated checks.

...
Every process event must carry bounded event authority derived from `RunAuthority`: `journeyId`, `runId`, `turnId`, `threadId`, `generation`, `piSessionId` and `mirrorConversationId`. The private `piSessionFile` path stays inside `RunAuthority` and backend settlement, not process event authority. Frontend reducers must reject late events, events without authority, events for replaced runs and events whose generation or session no longer matches the owning Journey conversation.

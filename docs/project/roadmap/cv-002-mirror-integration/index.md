...
| [CV-002.DS-004](ds-004-conversation-and-mirror-context-parity/index.md) | Conversation and Mirror Context Parity | The visible conversation, Pi-owned context budget/compaction, active Journey, identity, ego/persona routing and Mirror operating mode are the context lifecycle Pi actually uses to answer | 🟠 In Validation |

## Done Condition

CV-002 is done when Mirror is the Harness integration substrate in behavior, not only storage: Journeys and conversations can be imported and refreshed intentionally; the essential Pi/Mirror command-and-response loop is projected faithfully and settles correctly; the conversation visible in Nautilus is present in live agent context; Pi's authoritative context usage and automatic compaction lifecycle are visible and preserve continuity; Journey and identity/persona context are loaded before answering; and Mirror, Builder, Explorer and Soul modes preserve their existing surfaces and behavioral boundaries.

## Boundary

Mirror integration remains explicit. Nautilus must not continuously sync, silently mutate Mirror, invoke Pi automatically, persist secrets, or grant arbitrary filesystem authority. The current exception is user-triggered generated title updates for a selected Mirror conversation, delegated to Mirror's own title service.

...
Mirror Desktop is a usable desktop cockpit for operating Pi-backed agent conversations in local work.
...
| [DS-012](../ds-012-durable-turn-lifecycle/index.md) | Durable Turn Lifecycle | Mirror Desktop gives every dedicated turn one durable, idempotent lifecycle authority across execution, projection, Mirror delivery and restart recovery | 🟢 Done |

## Done Condition

CV-001 is complete when Mirror Desktop can be used as a practical local agent cockpit: sessions are explicit, provider setup is visible, Journey-bound runs are independently controllable and may proceed concurrently, useful Journey conversations can be reopened, Journeys can be searched/ordered/pinned and administered canonically, non-sensitive global agent defaults and per-Journey model/thinking overrides can be retained locally, the GUI is coherent in daily use, local files can be attached deliberately, and development can proceed in an unmistakable isolated desktop channel without disturbing the stable user app.

## Boundary

CV-001 itself does not integrate the full Nautilus Method, execute Missions, invoke Mirror, automate file mutation, package a public release or create multi-user collaboration. Later Capability Values separately introduced Mirror-mediated invocation and release boundaries.

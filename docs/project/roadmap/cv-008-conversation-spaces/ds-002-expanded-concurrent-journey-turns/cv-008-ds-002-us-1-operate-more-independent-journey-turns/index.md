...
Each running Journey continues to communicate its own existing sidebar and conversation state. Mirror Desktop adds no global process or turn counter. Only actionable full-capacity refusal appears, while the fifth Journey's draft stays editable for deliberate retry.
...
- Explicit English-only notice when a selected send reaches full capacity.
- Existing deliberate retry behavior with the unsent draft preserved.

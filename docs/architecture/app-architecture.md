...
CV-008.DS-002 extends that proven contract to exactly four admitted Journey leases. Existing owner-specific Journey states remain the ordinary presentation; the conversation surface adds no global process counter. Finalizing leases continue to consume admission until exact cleanup and fresh reinspection, and a fifth Journey receives an actionable refusal with Send disabled and no process, turn, message or queue creation while its draft remains editable. Steering continues through the owning process and consumes no additional slot. Growth above four requires separately governed product need and new resource and authority evidence.

...
At the US-1 checkpoint, execution is still globally serial at capacity 1 and the native child slot plus untargeted `cancel_pi_invocation` command remain unchanged. Journey and read-only altitude selection stay available while one owner streams or finalizes, but Send, Enter, attachments, restart, repair, settings mutation and Journey administration remain governed by aggregate occupancy. The owner row projects compact running/finalizing state from its keyed runtime entry, and cancellation is rendered only when that owner is selected. A full, identity- and generation-bound conversation snapshot lives transiently in the active/finalizing runtime entry so returning to the owner never runs startup interruption recovery against a live process. Live snapshot acceptance also requires the exact reconciliation turn, run and Harness user/assistant message evidence from `RunAuthority`; same-generation snapshots from another run are quarantined. Non-owner conversations load read-only while global occupancy is held; native transcript recovery that could persist an interruption is deferred until occupancy releases. After finalization, ordinary persisted restoration becomes authoritative again.

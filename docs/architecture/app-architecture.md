...
Settlement and persistence use captured run-start authority for transcript inspection, Harness projection, Mirror append, outbox acknowledgement and save ordering. Saves and finalization remain serial. Tests inject only bounded registry limits; production capacity is one private constant exactly equal to 1 with no environment override. Capacity 2 remains future US-2 work after TS-4 hardens persistence for overlap.

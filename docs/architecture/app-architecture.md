...
Presentation finalization cannot release native occupancy. Initial completion and matching retry share one dependency-injected boundary ordered as exact projection save, durable outbox enqueue, directed cleanup, then append/acknowledgement. Without an existing durable outbox, every retry repeats the exact projection save before enqueue; save or enqueue failure prevents cleanup. An existing exactly matched outbox may proceed directly to cleanup and append/acknowledgement. Append and acknowledgement may remain pending. Cancelled or failed turns request cleanup only after durable interrupted-state save. A rejected reservation restores reversible staging durably before inspection can re-enable admission. Any earlier failure or ambiguous inspection/cleanup response retains the lease and blocks operations.

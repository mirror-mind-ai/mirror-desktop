...
Reservation, duplicate-Journey checking, production-limit checking and insertion occur under one mutex before worker or child spawn. Each entry stores a cloneable child handle: directed control clones it after exact target validation and releases the registry mutex before `kill` or nonblocking `try_wait`; joins, event emission, filesystem access and post-processing also remain outside that mutex. Process capacity and Journey occupancy end at the same native terminal frontier: the first terminal signal releases child capacity and moves the inspectable entry to `finalizing`, whose non-open terminal state is debt rather than active execution. A competing reservation safely retires bounded finalization debt; remembered exact targets make late cleanup idempotent without touching a successor. First-terminal-wins prevents duplicate completion and capacity release. Spawn failure, process death, early cancel, cancel/done race, repeated terminalization and late A1 callbacks after A2 replacement are explicit deterministic paths.
...
Presentation finalization does not own native occupancy. Native terminalization itself releases Journey occupancy and process capacity; projection save, durable outbox enqueue, exact directed cleanup and fresh inspection remain authority-bound settlement work but cannot turn a non-open `finalizing` entry back into active execution. Without an existing durable outbox, pre-frontier transcript, projection, save and enqueue revalidate the active generation plus exact current run/turn after every await; rollover stops later effects and cleanup. Save or enqueue failure retains inspectable finalization debt, not admission authority. Append, receipt-save and acknowledgement are downstream model-free work and may remain pending while a successor runs.

Post-frontier recovery validates the exact generation-scoped projection and durable outbox rather than requiring the original generation active. Receipt-save reloads the latest generation projection, then the native Journey/generation lock uses the persisted projection as merge base and copies only the exact target turn's compatible Mirror receipt plus a monotonic checkpoint. A successor persisted between frontend load and native save therefore remains intact across messages, reconciliation turns, terminal/Steering evidence and metadata. Identical receipts are idempotent; contradictory receipt, turn or outbox authority fails closed. Frontend publication reloads durable state and proceeds only while both visible and persisted current-turn authority still match the settling run. Exact settlement diagnostics are keyed by Journey, run and turn, so an older success clears only its own debt and concurrent failures aggregate without replacing one another. Late work cannot release a replacement lease, overwrite a successor Conversation or remove another outbox item. Cancelled or failed turns request cleanup only after an exact journal-derived interrupted-state save. A rejected reservation restores reversible staging durably before inspection can re-enable admission. Ambiguous native inspection remains fail-closed; exact terminal finalization debt remains diagnosable but does not block a successor.

CR060 certifies this contract as one release-shaped 4 × 7 matrix: completed, cancelled, provider-failed and process-died outcomes crossed with projection, journal, Segment, outbox, Mirror, acknowledgement and presentation failures. Composed tests use the production occupancy, availability, exact-diagnostic, cleanup-reinspection and Pi-backed projection functions; released terminal debt never consumes capacity, while unknown or open execution remains fail-closed. `scripts/rs019_post_terminal_matrix.mjs` validates only a bounded DEV evidence document with exact coordinates and invariants, rejects arbitrary content or production identifiers, and emits counts plus a digest rather than prompts or responses. `npm run rehearsal:rs019:validate -- <evidence-path>` is the release-review command. Live fault injection and DEV app-data preparation remain separately authorized operations.

Retained occupancy exposes bounded inspection as live-execution evidence, while the journal alone chooses lifecycle recovery. `ConversationAvailability` does not accept journal readiness, and live submission has no separate retained-journal guard: `terminal_durable`, `projected`, `outbox_enqueued`, `settled` and `interrupted` evidence may remain visible without independently disabling Send. Unknown native inspection, an exact open reserved/running execution, process-local start reservation or selected live runtime still fail closed, and native start remains the final atomic journal-admission boundary. Recovery matches exact journal authority against persisted correlation, generation/thread, session, Mirror conversation and Harness message evidence; it starts no child and creates no run, turn, message, generation or staging. It resumes only the journal-recorded projection/save, outbox enqueue/append/acknowledgement or interrupted-save phase, then requests matching cleanup. Non-owner, stale and evidence-free recovery fail closed.
...
Native projection saves use bounded Journey/generation lock stripes and explicit active pre-frontier versus generation-scoped post-frontier modes. Active pre-frontier validation requires the authorized run/turn to be the exact latest reconciliation turn in both the candidate and the persisted projection read under the Journey/generation lock, not merely present in either history. Save modes preserve every persisted reconciliation turn plus committed Mirror receipts and checkpoints monotonically, so an authority-free lifecycle autosave cannot delete a pending replacement or regress acknowledged evidence. Exact rejected-reservation rollback uses its own authority-bound mode to remove only the rejected current turn. Only after that durable rollback and bounded reinspection succeed does the frontend remove the exact rejected runtime entry and staged snapshot; occupied sibling Journeys remain byte-for-byte unchanged. Rollback or inspection failure retains the rejected entry and diagnostic as fail-closed evidence. Authority conflicts fail closed. Saves publish through a unique staged sibling, file `sync_all`, atomic rename and parent-directory `sync_all`; failure preserves the prior projection. Exact enqueue is idempotent, inserted/existing receipts converge without repeated checkpoint increments, and acknowledgement returns `acknowledged` or `already_acknowledged` only from persisted exact proof. When an enqueue, legacy replacement or legacy normalization rejects a same-`itemId` payload as `mirror_append_item_conflict`, the native layer first records both payloads, the conflict site and the differing top-level and per-message keys in `mirror-append-conflicts.jsonl` beside the outbox, bounded to the most recent 64 records; the error code and the outbox are unchanged, and `list_mirror_append_conflicts` exposes the records per Journey read-only (CR087). The native visible-request projection removes one leading `/skill:` line before envelope detection and strips the attachment block on the single-newline marker the frontend emits, so the user text delivered to Mirror and reconstructed on the surface equals what the Navigator typed; conflict semantics remain fail-closed. Restart recovers only projection/outbox work, never a dead child or generation activation. Tests inject only governed registry limits 1, 2 and 4. CV-008.DS-002 fixes production capacity at one private constant exactly equal to 4 with no environment or user override; four is the supported horizon rather than an automatic step toward the internal hard maximum. The native registry admits at most the configured bounded active executions globally and one open reserved or running entry per Journey; terminal finalization entries are inspectable bounded debt and are safely retired when a successor or capacity needs their slot. The frontend validates only inspection limits 1, 2 and 4 and derives selected-Journey submission admission separately from the aggregate guard retained for settings, Journey administration, generation restart and selected-global attachment staging. Four occupied Journeys remain navigable and draft-editable; a fifth keeps its editable unsent draft, receives explicit immediate refusal and cannot Send until capacity frees, with no queue or automatic retry. Native reservation remains the final atomic TOCTOU boundary. App shutdown clones at most four exact running child handles under the registry lock and passes them to bounded outside-lock control; every captured handle is attempted even when an earlier control fails. The capacity-four expansion preserves the journal authority, event, reducer, persistence and command contracts proven at lower capacities; shutdown drains at most four exact child handles.
...
CV-008.DS-002 extends that proven contract to exactly four admitted active Journey leases. Existing owner-specific Journey states remain the ordinary presentation; the conversation surface adds no global process counter. Open reserved/running executions consume admission, while non-open finalizing entries are bounded debt and may be retired safely for a successor. A fifth active Journey receives an actionable refusal with Send disabled and no process, turn, message or queue creation while its draft remains editable. Steering continues through the owning process and consumes no additional slot. Growth above four requires separately governed product need and new resource and authority evidence.

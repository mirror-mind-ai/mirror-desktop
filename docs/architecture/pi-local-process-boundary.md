...
Frontend inspection accepts only the enabled limit 2 and rollback limit 1, validates entry/capacity consistency and derives selected-Journey admission separately from aggregate operational mutation guards. A free Journey can submit while one slot is occupied; an occupied Journey or a third Journey fails closed with an owner-correct reason, and native reservation remains the final atomic TOCTOU boundary. Global settings, Journey administration, generation restart and selected-global attachment staging retain their aggregate guards. Targeted cancellation captures one immutable live identity and sends only its exact `journeyId + runId`; navigation after the command begins cannot retarget native control or interrupted settlement. First-terminal classification is retained per run, and owner-keyed frontend updates cannot mutate a sibling runtime. App shutdown snapshots at most two exact running child handles under the registry lock, releases the lock, and then attempts bounded control of every captured handle even when an earlier control fails.

Rollback is capacity-only: the registry accepts injected limits 1 and 2 in tests, production capacity is defined by one private constant, there is no arbitrary environment override, and rollback changes only that constant from 2 to 1 without removing `journeyId` plus `runId` commands, correlated process events, Journey-keyed frontend state or captured-authority settlement.

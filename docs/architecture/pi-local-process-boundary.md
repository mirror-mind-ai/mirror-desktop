...
Persistence phases are FIFO-keyed per Journey with duplicate exact-turn convergence and independent Journey queues; provider lifetime is not held by the queue. Native active pre-frontier save requires the authorized run/turn to be the latest reconciliation turn, while generation-scoped post-frontier save may target exact historical evidence. Bounded Journey/generation lock stripes also preserve persisted committed Mirror receipts and checkpoints across stale authority-free lifecycle autosaves. Saves use exact authority validation, a unique staged sibling, file sync, atomic rename and parent sync. Remote Mirror append holds no global outbox-file lock. Exact acknowledgement is idempotent only with persisted committed proof and cannot remove another turn's item. Restart resumes exact projection/outbox recovery without restoring a child or activating a generation. Durable local projection plus outbox enqueue authorizes cleanup; exact cleanup plus fresh inspection releases the Journey for one later serial invocation even when prior append/ack remains recoverably pending.

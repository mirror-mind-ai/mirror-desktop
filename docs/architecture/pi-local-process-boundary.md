...
The backend process registry is keyed by `journeyId`. Each entry owns one immutable `RunAuthority`, one exact `runId`, an optional child process, cancellation and first-terminal state, a Journey lease and one private provider configuration snapshot. `start_pi_invocation(prompt, config, runAuthority)` remains the only start boundary. Cancel and cleanup require exact `journeyId + runId`; stale pairs cannot mutate or remove a replacement. After DS-012 serial settlement, restart recovery and TS-3 authority isolation passed, US-3 restored the private production constant to exactly 2; pure tests retain bounded limits 1 and 2 through the same registry implementation. A second open reserved or running lease for the same Journey is rejected atomically. A non-open finalizing entry remains inspectable after child capacity is released but no longer occupies the Journey or an admitted slot. Exact cleanup may remove it, while a competing reservation safely retires bounded debt and remembers the old target so late cleanup is idempotent and cannot affect the successor. More than two children remain impossible.

Native lifecycle is explicit: reserve `journeyId + runId` and claim capacity in one registry lock before worker or child spawn; attach only to the matching reservation; preserve an early cancel request through attachment; release process capacity and Journey occupancy on the first terminal signal; retain bounded inspectable finalization debt; and remove it through exact cleanup or safe retirement when a reservation needs its slot. Spawn failure, process death, cancel/done races, repeated terminalization and stale callbacks all use the same expected-run comparison. The first terminal transition wins. The worker joins output readers, stores only minimal exact Pi execution evidence (final assistant output, truncation state, IDs, count and timestamps) in the durable turn journal, and only then may emit native `done`; prompts, protocol streams, encrypted reasoning and stderr are not journaled. Journal failure retains inspectable finalization debt and emits a diagnostic, but cannot restore active occupancy after native terminalization. The registry stores a cloneable child handle. Cancel validates the exact target and clones that handle under the registry lock, then releases the registry before taking the child lock and issuing `kill`; the worker performs `try_wait` through its cloned child handle without taking the registry lock. Output-thread joins, emission, filesystem access and post-processing also remain outside the registry lock before exact terminalization returns to the registry.
...
Native occupancy and frontend presentation are separate. `finalization_finished` may end Recording but does not decide occupancy; exact native terminalization does. Completed turns retain an authority-bound settlement frontier for projection save, durable outbox enqueue, directed cleanup and fresh inspection, but failure at those secondary phases leaves non-blocking finalization debt. Retry repeats the active exact save when no durable outbox exists; an existing exactly matched outbox skips save/enqueue. Mirror append, generation-scoped receipt-save and acknowledgement are downstream and may remain pending while a successor runs. Cancelled and failed turns request cleanup only after exact journal-derived interrupted-state save. A reversible staging loser is durably rolled back before bounded inspection may restore admission because it never crossed the terminal frontier. Successful rollback/reinspection then removes only that exact rejected frontend runtime entry and staged snapshot; sibling runtime entries are preserved byte-for-byte. Unknown or malformed inspection remains fail-closed; exact terminal finalization, projection, enqueue, interrupted-save or cleanup debt does not represent active execution.

A retained terminal entry permits bounded inspection and diagnostics without disabling the Composer; inspection is live-execution evidence only for exact open reserved/running entries rather than lifecycle authority. Journal recovery starts no child, creates no run, turn, message, generation or staging, and resumes only projection/save, outbox enqueue/append/acknowledgement or interrupted-state save for exact journal authority matching Journey, run, turn, thread, generation, session, Mirror conversation and Harness message IDs. Non-owner, stale or journal-free recovery fails closed.
...
Frontend inspection accepts only governed limits 1, 2 and the fixed production limit 4, validates entry/capacity consistency and treats occupancy as presentation/live-execution evidence; native journal admission remains the final atomic lifecycle and TOCTOU boundary. Global settings, Journey administration, generation restart and selected-global attachment staging retain their aggregate guards. Targeted cancellation captures one immutable live identity and sends only its exact `journeyId + runId`; navigation after the command begins cannot retarget native control or interrupted settlement. First-terminal classification is retained per run, and owner-keyed frontend updates cannot mutate a sibling runtime. App shutdown snapshots at most four exact running child handles under the registry lock, releases the lock, and then attempts bounded control of every captured handle even when an earlier control fails.

CV-008.DS-002 sets capacity four through the private production constant, with no user or environment override and no automatic path toward the internal hard maximum. A fifth Journey is refused immediately and keeps its draft for deliberate retry; no prompt queue or background retry exists. Finalizing entries no longer occupy admission after their child process reaches an exact terminal state and releases process capacity; durable projection, outbox enqueue, cleanup and reinspection remain secondary settlement evidence. Steering reuses its owning process and consumes no additional slot. A one-line return to capacity two remains the operational rollback without changing correlated commands, events, Journey-keyed state, persistence or restart authority.
...
- Per-Journey ownership remains one reserved, active or finalizing run per Journey; CV-008.DS-002 expands production global admission to exactly 4.
- Capacity remains exactly 4 unless separately governed future need justifies new evidence. The lower-capacity selective cancellation, controlled failure-frontier, sibling-isolation and model-free recovery contracts remain unchanged, and expanded validation stays isolated from stable app-data and production Mirror.

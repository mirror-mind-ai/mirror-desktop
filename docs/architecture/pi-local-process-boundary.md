...
CV-008.DS-002 sets capacity four through the private production constant, with no user or environment override and no automatic path toward the internal hard maximum. A fifth Journey is refused immediately and keeps its draft for deliberate retry; no prompt queue or background retry exists. Finalizing leases continue to occupy admission after their child process releases process capacity and become available only after durable projection, outbox enqueue, exact cleanup and fresh reinspection. Steering reuses its owning process and consumes no additional slot. A one-line return to capacity two remains the operational rollback without changing correlated commands, events, Journey-keyed state, persistence or restart authority.

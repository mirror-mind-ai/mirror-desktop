...
Runtime validation discovers tools without invoking a login shell or reading shell startup files. It inspects a bounded allowlist of system locations and supported user managers (NVM, FNM, Volta, asdf and mise), requires one unambiguous Pi installation paired with Node, canonicalizes executable targets and prepends the verified tool directories to the child-only `PATH`. `uv` remains independently required from the same bounded search surface. Missing or ambiguous tools prevent binding persistence and Journey import; no global graphical environment is trusted or modified.

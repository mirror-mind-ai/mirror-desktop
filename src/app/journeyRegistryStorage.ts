...
export async function refreshJourneyRegistry(activeJourneyId: string): Promise<JourneyRegistry> {
  const payload = await invoke<string>("refresh_journey_registry", { activeJourneyId });
  try {

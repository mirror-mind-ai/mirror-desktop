...
export function planPrivateUpdatePublication({ version, currentVersions, targets = defaultTargets, baseUrl = defaultBaseUrl }) {
...
function parseArguments(argv) {
  const args = {
    ...
    publish: false,
    sshHost: "szen-vps",
    webRoot: "/var/www/mirror-desktop-updates/mirror-desktop",
  };
...
export function stagePrivateUpdatePublication(options) {
  ...
  const plan = planPrivateUpdatePublication({ version, currentVersions: options.currentVersions, targets: options.targets, baseUrl: options.baseUrl });
...
export function publishStagedPrivateUpdate({ stageDir, sshHost = "szen-vps", webRoot = "/var/www/mirror-desktop-updates/mirror-desktop", manifestPaths }) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    if (options.publish) publishStagedPrivateUpdate({ ...plan, sshHost: options.sshHost, webRoot: options.webRoot });
